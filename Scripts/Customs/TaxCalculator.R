...
clusterExport(cl, c(
  ...
  "cal_weighting_fun", "vec_calc_customs_duties_fun"
))
...
results <- parLapply(cl, seq_along(scenarios), function(scenario_index) {
  ...
  # Define current year based on scenario index
  scenario <- scenarios[scenario_index]
  current_year <- base_year + (scenario_index - 1)
  
  # Extract weights and ensure it matches sample data length
  ...
  # Extract growth factors for the current year and scenario
  ...
  growth_factors <- as.numeric(gf_row[, -c("Year", "scenarios")])
  
  # Set the dataset for Customs_BU (always using customs_simulation_parameters_raw)
  selected_parameters_BU <- customs_simulation_parameters_raw
  
  # Ensure the merge happens on a valid common column (HS_code)
  merged_data_BU <- merge(customs_data$sample, selected_parameters_BU[, .(HS_code, Effective_Customs_rate)], by = "HS_code", all.x = TRUE)
  
  # Weighting and custom calculations for Customs_BU
  columns_to_process_BU <- setdiff(colnames(merged_data_BU), c("HS_code", "Effective_Customs_rate", "Value_adjusted"))
  ...
  # II. Process Customs_SIM ----
  # Determine the correct parameters for Customs_SIM based on simulation year
  selected_parameters_SIM <- get_sim_dataset(current_year, simulation_year)
  
  # Merge with dynamically selected parameters for SIM
  merged_data_SIM <- merge(customs_data$sample, selected_parameters_SIM[, .(HS_code, Effective_Customs_rate)], by = "HS_code", all.x = TRUE)
  
  # Weighting and custom calculations for Customs_SIM
  columns_to_process_SIM <- setdiff(colnames(merged_data_SIM), c("HS_code", "Effective_Customs_rate", "Value_adjusted"))
...
# Compile results from each scenario
...
rm(results)
        ...
        #   2. Aggregate Customs revenues by years  (Customs_summary_df) ---------------------------------------------
        ...
        #   select(year, "Current law (LCU Mil)", "Simulation (LCU Mil)", "Fiscal impact (LCU Mil)", GDP) %>%
        #   dplyr::mutate(
        #     `Current law (Pct of GDP)` = round(`Current law (LCU Mil)` / GDP * 100, 2),
        #     `Simulation (Pct of GDP)` = round(`Simulation (LCU Mil)` / GDP * 100, 2),
        #     `Fiscal impact (Pct of GDP)` = round(`Fiscal impact (LCU Mil)` / GDP * 100, 2)
        #   ) %>%
        #   dplyr::select(-c(GDP))
