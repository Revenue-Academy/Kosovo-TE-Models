      ...
      # 2.Main Simulation Results ----------------------------------------------
                ...
                MainResultsCustoms1<-MacroFiscalData%>%
                  ...
                  dplyr::mutate(
                    ...
                    `Import(without FTI)`=sum(CustomsDuties_base_non_preferential$Value,na.rm=TRUE)/1e+06, 
                    `Actual Customs Revenues(without FTI)`=sum(CustomsDuties_base_non_preferential$CustomsRevenue,na.rm=TRUE)/1e+06, 
                    `Effective Customs Rate (%) (without FTI)`=sum(CustomsDuties_base_non_preferential$CustomsRevenue,na.rm=TRUE)/sum(CustomsDuties_base_non_preferential$Value,na.rm=TRUE)*100,
                    
                    `Customs Revenue Benchmark(without FTI)`=sum(CustomsDuties_base_non_preferential$CustomsDuties_Benchmark,na.rm=TRUE)/1e+06,
                    `Benchmark Effective Customs Rate(without FTI)`=`Customs Revenue Benchmark(without FTI)`/(sum(CustomsDuties_base_non_preferential$Value,na.rm=TRUE)/1e+06)*100,
                    `Tax Expenditures(without FTI)`=sum(CustomsDuties_base_non_preferential$CustomsDuties_TE,na.rm=TRUE)/1e+06,
                    `Tax Expenditures(without FTI) as % of GDP`=(`Tax Expenditures(without FTI)`/GDP)*100,
                    `Tax Expenditures(without FTI) as % of import FOB`=(`Tax Expenditures(without FTI)`/ImportsOfGoods_FOB)*100,
                    `Tax Expenditures(without FTI) as % of Government Revenue`=(`Tax Expenditures(without FTI)`/GeneralGovernmentRevenue)*100,
                    `Tax Expenditures (as % of Taxes On Products)`=(`Tax Expenditures(without FTI)`/TaxesOnProducts)*100,
                    `Tax Expenditures (as % of CustomsRevenue)`=(`Tax Expenditures(without FTI)`/ImportDuties)*100)%>%
                  dplyr::select(
                    `Actual Total Import`,
                    `Actual Total Customs Revenues`,
                    `Actual Total Effective Customs Rate (%)`,
                    `Import(without FTI)`,
                    `Actual Customs Revenues(without FTI)`,
                    `Effective Customs Rate (%) (without FTI)`,
                    `Customs Revenue Benchmark(without FTI)`,
                    `Benchmark Effective Customs Rate(without FTI)`,
                    `Tax Expenditures(without FTI)`,
                    `Tax Expenditures(without FTI) as % of GDP`,
                    `Tax Expenditures(without FTI) as % of import FOB`,
                    `Tax Expenditures(without FTI) as % of Government Revenue`,
