...
actual_year_simulation <- simulation_year
...
#   select(Month,Treatment)
                                          ...
                                          CustomsDuties_TE_agg_countries <- CustomsDuties_TE_agg_countries %>%
                                            dplyr::select(Year,iso3c,CustomsDuties_TE)%>%
                                            dplyr::group_by(Year,iso3c) %>%
                                            dplyr::summarise(across(where(is.numeric), sum))

                                          CustomsDuties_TE_agg_HS<-CustomsDuties_base%>%
                                            dplyr::group_by(Year,HS_code,Treatment,HS_code_s)%>%
                                            dplyr::filter(Treatment=="NonPreferential")%>%
                                            dplyr::summarise(Value=sum(Value,na.rm = TRUE),
                                                             Quantity=sum(Quantity,na.rm = TRUE),
                                                             Netweight=sum(Netweight,na.rm = TRUE),
                                          ...
                                          #CustomsDuties_TE_agg_HS<-left_join(CustomsDuties_TE_agg_HS,taric_data,by =c("HS_code","Year"))
                                          
                                          CustomsDuties_TE_agg_HS <- left_join(
                                                                                CustomsDuties_TE_agg_HS,
                                                                                taric_data,
                                                                                by = c("HS_code" = "HS_code", "Year" = "Year")
                                                                              )
          ...
          # 2.Preparing data for charts and for the main table --------------------------------------------
                                ...
                                CustomsValue<-CustomsDuties_TE_agg_HS%>%
                                  dplyr::select(Year,Chapter,Value)%>%
                                  dplyr::group_by(Year,Chapter)%>%
                                  dplyr::summarise(Value=sum(Value,na.rm = TRUE))

                                CustomsValue<-left_join(CustomsValue,HS_Sections,by =c("Chapter"))%>%
                                  dplyr::select(Year,Chapter,Chapters_description,Value)%>%
                                ...
                                CustomsDuties_base_pie<-CustomsDuties_base%>%
                                  dplyr::select(Year,Treatment,Value)%>%
                                ...
                                CustomsDuties_TE_type_products<-CustomsDuties_TE_agg_HS%>%
                                  dplyr::select(Year,Chapter,Treatment,CustomsDuties_TE)%>%
                                  dplyr::group_by(Year,Chapter,Treatment)%>%
                                ...
                                CustomsDuties_TE_Agricultural<-CustomsDuties_TE_Agricultural%>%
                                  select(Year,Treatment,type_products,CustomsDuties_TE)%>%
                                  dplyr::group_by(Year,Treatment,type_products)%>%
                                  dplyr::summarise(CustomsDuties_TE=sum(CustomsDuties_TE,na.rm = TRUE))
...
# 
                                ...
                                CustomsDuties_TE_Sections<-left_join(CustomsDuties_TE_Sections,HS_Sections,by =c("Chapter"))%>%
                                  dplyr::select(Year,Sections,Section_description,Treatment,CustomsDuties_TE)%>%
                                  dplyr::group_by(Year,Sections,Section_description,Treatment)%>%
                                  dplyr::summarise(CustomsDuties_TE=sum(CustomsDuties_TE,na.rm = TRUE))%>%
                                  data.table()
                                ...
                                CustomsDuties_TE_Chapters<-left_join(CustomsDuties_TE_Chapters,HS_Sections,by =c("Chapter"))%>%
                                  dplyr::select(Year,Chapter,Chapters_description,Treatment,CustomsDuties_TE)%>%
                                  dplyr::group_by(Year,Chapter,Chapters_description,Treatment)%>%
                                  dplyr::summarise(CustomsDuties_TE=sum(CustomsDuties_TE,na.rm = TRUE))
                                ...
                                CustomsDuties_TE_MTN<-CustomsDuties_TE_agg_HS%>%
                                  dplyr::select(Year,Six_digit,Treatment,CustomsDuties_TE)%>%
                                  dplyr::group_by(Year,Six_digit,Treatment)%>%
                                  dplyr::summarise(CustomsDuties_TE=sum(CustomsDuties_TE,na.rm = TRUE))
                                ...
                                CustomsDuties_TE_MTN<-left_join(CustomsDuties_TE_MTN,WTO_MTN_subset,by =c("Six_digit"))%>%
                                  dplyr::select(Year,Product_group,Treatment,CustomsDuties_TE)

                                CustomsDuties_TE_MTN <- distinct(CustomsDuties_TE_MTN)

                                CustomsDuties_TE_MTN<-CustomsDuties_TE_MTN%>%
                                  dplyr::group_by(Year,Product_group,Treatment)%>%
                                  dplyr::summarise(CustomsDuties_TE=sum(CustomsDuties_TE,na.rm = TRUE))
                               ...
                               # CustomsDuties_TE_agg_countries_tbl<-left_join(CustomsDuties_TE_agg_countries_tbl,GeoDimension, by=c("iso3c"))%>%
                                ...
                                CustomsDuties_TE_agg_HS_subset<-CustomsDuties_base%>%
                                  dplyr::select(Year,HS_code_s,Description,Treatment,CustomsDuties_TE)%>%
                                  dplyr::filter(Treatment=="NonPreferential")%>%
                                  dplyr::group_by(Year,HS_code_s,Description,Treatment)%>%
          ...
          #               
                                ...
                                # Harmonize Year type everywhere used in joins
                                MacroFiscalData_INT <- MacroFiscalData %>%
                                  dplyr::mutate(Year = as.integer(as.character(Year)))
                                
                                CustomsDuties_base <- CustomsDuties_base %>%
                                  dplyr::mutate(Year = as.integer(Year))
                                
                                CustomsDuties_TE_agg_HS <- CustomsDuties_TE_agg_HS %>%
                                  dplyr::mutate(Year = as.integer(Year))
                                ...
                                customs_total_year <- CustomsDuties_base %>%
                                  dplyr::group_by(Year) %>%
                                  dplyr::summarise(
                                    `Actual Total Import` = sum(Value, na.rm = TRUE) / 1e06,
                                    `Actual Total Customs Revenues` = sum(CustomsRevenue, na.rm = TRUE) / 1e06,
                                    .groups = "drop"
                                  ) %>%
                                  dplyr::mutate(
                                    `Actual Total Effective Customs Rate (%)` = dplyr::if_else(
                                      `Actual Total Import` == 0,
                                      0,
                                      (`Actual Total Customs Revenues` / `Actual Total Import`) * 100
                                    )
                                  )
                                
                                customs_nonpref_year <- CustomsDuties_base_non_preferential %>%
                                  dplyr::group_by(Year) %>%
                                  dplyr::summarise(
                                    `Import(without FTA)` = sum(Value, na.rm = TRUE) / 1e06,
                                    `Actual Customs Revenues(without FTA)` = sum(CustomsRevenue, na.rm = TRUE) / 1e06,
                                    `Customs Revenue Benchmark(without FTA)` = sum(CustomsDuties_Benchmark, na.rm = TRUE) / 1e06,
                                    `Tax Expenditures(without FTA)` = sum(CustomsDuties_TE, na.rm = TRUE) / 1e06,
                                    .groups = "drop"
                                  ) %>%
                                  dplyr::mutate(
                                    `Effective Customs Rate (%) (without FTA)` = dplyr::if_else(
                                      `Import(without FTA)` == 0,
                                      0,
                                      (`Actual Customs Revenues(without FTA)` / `Import(without FTA)`) * 100
                                    ),
                                    `Benchmark Effective Customs Rate(without FTA)` = dplyr::if_else(
                                      `Import(without FTA)` == 0,
                                      0,
                                      (`Customs Revenue Benchmark(without FTA)` / `Import(without FTA)`) * 100
                                    )
                                  )
                                
                                MainResultsCustoms_wide <- MacroFiscalData_INT %>%
                                  dplyr::filter(Year %in% sort(unique(CustomsDuties_base$Year))) %>%
                                  dplyr::select(
                                    Year,
                                    GDP,
                                    ImportsOfGoods_FOB,
                                    GeneralGovernmentRevenue,
                                    TaxesOnProducts,
                                    ImportDuties
                                  ) %>%
                                  dplyr::left_join(customs_total_year, by = "Year") %>%
                                  dplyr::left_join(customs_nonpref_year, by = "Year") %>%
                                  dplyr::mutate(
                                    `Actual Total Import` = dplyr::coalesce(`Actual Total Import`, 0),
                                    `Actual Total Customs Revenues` = dplyr::coalesce(`Actual Total Customs Revenues`, 0),
                                    `Actual Total Effective Customs Rate (%)` = dplyr::coalesce(`Actual Total Effective Customs Rate (%)`, 0),
                                    `Import(without FTA)` = dplyr::coalesce(`Import(without FTA)`, 0),
                                    `Actual Customs Revenues(without FTA)` = dplyr::coalesce(`Actual Customs Revenues(without FTA)`, 0),
                                    `Effective Customs Rate (%) (without FTA)` = dplyr::coalesce(`Effective Customs Rate (%) (without FTA)`, 0),
                                    `Customs Revenue Benchmark(without FTA)` = dplyr::coalesce(`Customs Revenue Benchmark(without FTA)`, 0),
                                    `Benchmark Effective Customs Rate(without FTA)` = dplyr::coalesce(`Benchmark Effective Customs Rate(without FTA)`, 0),
                                    `Tax Expenditures(without FTA)` = dplyr::coalesce(`Tax Expenditures(without FTA)`, 0),
                                    `Tax Expenditures(without FTA) as % of GDP` = dplyr::if_else(
                                      is.na(GDP) | GDP == 0,
                                      0,
                                      (`Tax Expenditures(without FTA)` / GDP) * 100
                                    ),
                                    `Tax Expenditures(without FTA) as % of import FOB` = dplyr::if_else(
                                      is.na(ImportsOfGoods_FOB) | ImportsOfGoods_FOB == 0,
                                      0,
                                      (`Tax Expenditures(without FTA)` / ImportsOfGoods_FOB) * 100
                                    ),
                                    `Tax Expenditures(without FTA) as % of Government Revenue` = dplyr::if_else(
                                      is.na(GeneralGovernmentRevenue) | GeneralGovernmentRevenue == 0,
                                      0,
                                      (`Tax Expenditures(without FTA)` / GeneralGovernmentRevenue) * 100
                                    ),
                                    `Tax Expenditures (as % of Taxes On Products)` = dplyr::if_else(
                                      is.na(TaxesOnProducts) | TaxesOnProducts == 0,
                                      0,
                                      (`Tax Expenditures(without FTA)` / TaxesOnProducts) * 100
                                    ),
                                    `Tax Expenditures (as % of CustomsRevenue)` = dplyr::if_else(
                                      is.na(ImportDuties) | ImportDuties == 0,
                                      0,
                                      (`Tax Expenditures(without FTA)` / ImportDuties) * 100
                                    )
                                  ) %>%
                                  dplyr::select(
                                    Year,
                                ...
                                MainResultsCustoms1 <- MainResultsCustoms_wide %>%
                                  tidyr::pivot_longer(
                                    cols = -Year,
                                    names_to = "variable",
                                    values_to = "value"
                                ...
                                GDP_share_TE <- MainResultsCustoms_wide %>%
                                  dplyr::select(Year, `Tax Expenditures(without FTA) as % of GDP`) %>%
                                  dplyr::rename(GDP_share_TE = `Tax Expenditures(without FTA) as % of GDP`)
                                
                                ProjectionTE_customs <- MacroFiscalData_INT %>%
                                  dplyr::filter(Year >= base_year) %>%
                                  dplyr::select(Year, GDP) %>%
                                  dplyr::left_join(GDP_share_TE, by = "Year") %>%
                                  dplyr::mutate(
                                    GDP_share_TE = dplyr::coalesce(GDP_share_TE, 0),
                                    tax_expenditures = GDP * GDP_share_TE / 100
                                  )
...
# II. Excise ------------------------------------------------------------------
        ...
        # 1. Data Sub-setting-------------------------------------------------------
                      ...
                      HS_Chapter_27 <- HS_Chapter_27 %>%
                        dplyr::mutate(Subdataset = case_when(
                          ...
                          Eight_digit == "2710 19 44" ~ 'EURO DIESEL',
                          Eight_digit == "2710 19 48" ~ 'EURO DIESEL',
                      ...
                      Other_Ch_38 <- CustomsDuties_TE_agg_HS %>%
                        ...
                        #dplyr::filter(HS4_COD %in% c('3811','3814','3817'))
                     ...
                     #Cars_tbl<-left_join(CarsSet,Cars_ExciseRates, by=c("HS_code_s"="HS"))
                      ...
                      # new
                      # Cars_tbl<-Cars_tbl%>%
                      #   dplyr::mutate(PotentialExcise=Quantity*ExciseRate,
                      #                 TE=PotentialExcise-ExciseRevenue
                      #   )
        ...
        # 3. Main table with Simulation Results ----------------------------------------------------------------
                      ...
                      Legal_Fuels_Cars <- dplyr::bind_rows(Fuel_tbl, Cars_tbl) %>%
                        dplyr::mutate(Year = as.integer(Year))
                      
                      # Optional clamp: if you do not want negative legal TE by year, uncomment below
                      # Legal_Fuels_Cars <- Legal_Fuels_Cars %>%
                      #   dplyr::mutate(TE = pmax(TE, 0))
                      ...
                      excise_total_year <- CustomsDuties_TE_agg_HS %>%
                        dplyr::group_by(Year) %>%
                        dplyr::summarise(
                          `Actual Total Import` = sum(Value, na.rm = TRUE) / 1e06,
                          `Actual Total Excise Revenues` = sum(ExciseRevenue, na.rm = TRUE) / 1e06,
                          .groups = "drop"
                        )
                      
                      fuel_benchmark_year <- Fuel_tbl %>%
                        dplyr::group_by(Year) %>%
                        dplyr::summarise(
                          Fuel_tbl_Excise = sum(PotentialExcise, na.rm = TRUE) / 1e06,
                          Fuel_tbl_TE = sum(TE, na.rm = TRUE) / 1e06,
                          .groups = "drop"
                        )
                      
                      cars_benchmark_year <- Cars_tbl %>%
                        dplyr::group_by(Year) %>%
                        dplyr::summarise(
                          TE_Cars_ExciseTotal = sum(PotentialExcise, na.rm = TRUE) / 1e06,
                          Cars_tbl_TE = sum(TE, na.rm = TRUE) / 1e06,
                          .groups = "drop"
                        )
                      
                      legal_fuels_cars_year <- Legal_Fuels_Cars %>%
                        dplyr::group_by(Year) %>%
                        dplyr::summarise(
                          `Tax Expenditures` = sum(TE, na.rm = TRUE) / 1e06,
                          .groups = "drop"
                        )
                      
                      MainResultsExcise_wide <- MacroFiscalData_INT %>%
                        dplyr::filter(Year %in% sort(unique(CustomsDuties_TE_agg_HS$Year))) %>%
                        dplyr::select(Year, GDP, GeneralGovernmentRevenue, TaxesOnProducts) %>%
                        dplyr::left_join(excise_total_year, by = "Year") %>%
                        dplyr::left_join(fuel_benchmark_year, by = "Year") %>%
                        dplyr::left_join(cars_benchmark_year, by = "Year") %>%
                        dplyr::left_join(legal_fuels_cars_year, by = "Year") %>%
                        dplyr::mutate(
                          `Actual Total Import` = dplyr::coalesce(`Actual Total Import`, 0),
                          `Actual Total Excise Revenues` = dplyr::coalesce(`Actual Total Excise Revenues`, 0),
                          Fuel_tbl_Excise = dplyr::coalesce(Fuel_tbl_Excise, 0),
                          Fuel_tbl_TE = dplyr::coalesce(Fuel_tbl_TE, 0),
                          TE_Cars_ExciseTotal = dplyr::coalesce(TE_Cars_ExciseTotal, 0),
                          Cars_tbl_TE = dplyr::coalesce(Cars_tbl_TE, 0),
                          `Tax Expenditures` = dplyr::coalesce(`Tax Expenditures`, 0),
                          `Excise Revenue Benchmark` = TE_Cars_ExciseTotal + Fuel_tbl_Excise,
                          `Tax Expenditures as % of GDP` = dplyr::if_else(
                            is.na(GDP) | GDP == 0,
                            0,
                            (`Tax Expenditures` / GDP) * 100
                          ),
                          `Tax Expenditures as % of Government Revenue` = dplyr::if_else(
                            is.na(GeneralGovernmentRevenue) | GeneralGovernmentRevenue == 0,
                            0,
                            (`Tax Expenditures` / GeneralGovernmentRevenue) * 100
                          ),
                          `Tax Expenditures (as % of Taxes On Products)` = dplyr::if_else(
                            is.na(TaxesOnProducts) | TaxesOnProducts == 0,
                            0,
                            (`Tax Expenditures` / TaxesOnProducts) * 100
                          ),
                          `Tax Expenditures (as % of ExciseRevenue)` = dplyr::if_else(
                            `Actual Total Excise Revenues` == 0,
                            0,
                            (`Tax Expenditures` / `Actual Total Excise Revenues`) * 100
                          ),
                          `Tax Expenditures by Mineral Oils` = Fuel_tbl_TE,
                          `Tax Expenditures by Tobacco Products` = 0,
                          `Tax Expenditures by Alcohol Products` = 0,
                          `Tax Expenditures by Cars` = Cars_tbl_TE
                        ) %>%
                        dplyr::select(
                          Year,
                          `Actual Total Import`,
                      ...
                      MainResultsExcise_1 <- MainResultsExcise_wide %>%
                        tidyr::pivot_longer(
                          cols = -Year,
                          names_to = "variable",
                          values_to = "value"
                      ...
                      GDP_share_TE <- MainResultsExcise_wide %>%
                        dplyr::select(Year, `Tax Expenditures as % of GDP`) %>%
                        dplyr::rename(GDP_share_TE = `Tax Expenditures as % of GDP`)
                      
                      ProjectionTE_Excise <- MacroFiscalData_INT %>%
                        dplyr::filter(Year >= base_year) %>%
                        dplyr::select(Year, GDP) %>%
                        dplyr::left_join(GDP_share_TE, by = "Year") %>%
                        dplyr::mutate(
