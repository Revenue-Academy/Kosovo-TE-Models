...
# 1. TE-Special allowance for new assets ----------------------------------------------------------------------
...
cit_simulation_parameters_updated_te[3,7]<-0
          ...
          start_index <- match(base_year, scenario_years)# 1..5
          ...
          te_agg_rate_spl_all<-te_summary_df%>%
            dplyr::select(year, `tax expenditure`)
         


          
          
# 2. TE-Charitable contribution ----------------------------------------------------------------------
          cit_simulation_parameters_updated_te<-cit_simulation_parameters_raw
          ...
          te_agg_rate_max_charitable_ded<-te_summary_df%>%
            dplyr::select(year, `tax expenditure`)
          
         
          
          
# 3. TE-Discounts for sponsorship in the field of culture and youth ----------------------------------------------------------------------
          cit_simulation_parameters_updated_te<-cit_simulation_parameters_raw
          ...
          te_agg_rate_ded_culture_youth<-te_summary_df%>%
            dplyr::select(year, `tax expenditure`)
          

          
# 4. TE- Discounts for sponsorship in the field of sports ----------------------------------------------------------------------
          cit_simulation_parameters_updated_te<-cit_simulation_parameters_raw
          ...
          te_agg_rate_ded_sponsorship<-te_summary_df%>%
            dplyr::select(year, `tax expenditure`)
...
# 5.ALL TE ----------------------------------------------------------------
          ...
          start_index <- match(base_year, scenario_years) # <---Simulation year changed with base year
          ...
          te_summary_df <- te_summary_df %>%
            dplyr::mutate(across(where(is.numeric), ~ round(. / 1e03, 1)))
          
          
          
          # Aggregation by type of TE
          
          
          
          te_agg_rate_spl_all$type<-c("Special allowance for new assets")
          te_agg_rate_max_charitable_ded$type<-c("Charitable contribution")
          te_agg_rate_ded_sponsorship$type<-c("Discounts for sponsorship in the field of sports")
          te_agg_rate_ded_culture_youth$type<-c("Discounts for sponsorship in the field of culture and youth")
          
          
          te_summary_df_type<-rbind(te_agg_rate_spl_all,te_agg_rate_max_charitable_ded,te_agg_rate_ded_sponsorship,te_agg_rate_ded_culture_youth)
