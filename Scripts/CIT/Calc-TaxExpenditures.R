...
# Regardless of SimulationYear, we do the entire chain t0..t4 in CIT_BU_list_TE
...
sum(CIT_BU_list_TE$t0$calc_cit)
#sum(CIT_BU_list_TE$t0$totax)


sum(CIT_SIM_list_TE$t0$calc_cit)
#sum(CIT_SIM_list_TE$t0$totax)
...
# 9.Regions -----------------------------------------------------------------
          ...
          #   dt[, .(calc_cit, city, scenario = scenario)]
          # }
          # 
          # # Extract and process PIT_BU_list_TE
          # extracted_tables_bu <- mapply(extract_te_region_fun, CIT_BU_list_TE, scenarios, SIMPLIFY = FALSE)
          # combined_data_bu <- rbindlist(extracted_tables_bu)
          # result_bu <- combined_data_bu[, .(total_calc_citax_bu = sum(calc_cit)), by = .(scenario, city)]
          # 
          # # Extract and process PIT_SIM_list_TE
          # 
          # extracted_tables_sim <- mapply(extract_te_region_fun, CIT_SIM_list_TE, scenarios, SIMPLIFY = FALSE)
          # combined_data_sim <- rbindlist(extracted_tables_sim)
          # result_sim <- combined_data_sim[, .(total_calc_citax_sim = sum(calc_cit)), by = .(scenario, city)]
          # 
          # # Add year column to both results
          # result_bu[, year := forecast_horizon[match(scenario, scenarios)]]
          # result_sim[, year := forecast_horizon[match(scenario, scenarios)]]
          # 
          # # Combine both results into one data frame
          # region_cit_summary <- merge(result_bu, result_sim, by = c("scenario", "year","city"), all = TRUE)
          # 
          # 
          # region_cit_summary<-region_cit_summary%>%
          #   dplyr::mutate(tax_expenditures=total_calc_citax_sim-total_calc_citax_bu)
          # 
          # 
          # region_cit_summary_te<-region_cit_summary%>%
          #   select(year,city,tax_expenditures)
          # 
          # 
          # # Remove rows with NA in City for the plot
          # region_cit_summary_te <- region_cit_summary_te[!is.na(city)]
          # #region_cit_summary_te[is.na(City), City := "Other"]
          # 
          # region_cit_summary_te<-region_cit_summary_te%>%
          #   dplyr::filter(year==SimulationYear)
          # 
          # merged_df_regions_cit_regions<-left_join(region_cit_summary_te,region_data,by=c('city'='city'))
          # 
          # 
          # merged_df_regions_cit_regions<-merged_df_regions_cit_regions%>%
          #   dplyr::group_by(regions)%>%
          #   dplyr::summarise(tax_expenditures=sum(tax_expenditures)) 
          # 
          # 
          # choropleth_data <- merge(kosovo_regions, merged_df_regions_cit_regions, by.x = "nam", by.y = "regions", all.x = TRUE)
          # 
          # 
          # View(merged_df_regions_cit_regions)
          # 
