...
selected_gross_desc_tbl<-selected_gross_desc_tbl%>%
  filter(net_income_business>=0)


# selected_gross_desc_tbl <- selected_gross_desc_tbl %>%
#   rowwise() %>%
#   mutate(net_income_business = sum(ifelse(net_income_business < 0, 0, net_income_business)))


labor_capital <- selected_gross_desc_tbl %>%
  dplyr::group_by(decile_group) %>%
  dplyr::select(-c(total_inc_adjusted))%>%
  dplyr::summarize(across(c(gross_wage_w,gross_wage, net_income_business, net_income_business,net_income_partnership, gross_rents, gross_i_interest_pen_pay,
...
long_labor_capital_type <- labor_capital %>%
  ...
  dplyr::summarise( gross_wage_w  = sum(gross_wage_w , na.rm = TRUE),
                  gross_wage  = sum(gross_wage , na.rm = TRUE),
                   net_income_business = sum(net_income_business, na.rm = TRUE),
  ...
  #dplyr::mutate(gross_wage=gross_wage+gross_wage_w)
...
long_labor_capital_type$TypeOfIncome <- "In MIL LCU"
...
gross_nace_tbl <- selected_gross_nace_tbl %>%
  ...
  dplyr::summarise(total_inc = sum(calc_total_inc, na.rm = TRUE)
  )
...
gross_nace_tbl$TypeOfIncome <- "In MIL LCU"
