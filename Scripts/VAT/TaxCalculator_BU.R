suppressMessages({
  # II. SIMULATION ----
        ...
        # new 7/12/2024
            ...
            # 1.1.2 Estimation of effective VAT RATE ----------------------------------------------------------
  ...
  # 1.2 Benchmark revenues -----
            ...
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_HH[grepl("Imputed rent", CPA_PRODUCTS_BU$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
            ...
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_NPISH[grepl("Imputed rent", CPA_PRODUCTS_BU$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
            ...
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_Government[grepl("Imputed rent", CPA_PRODUCTS_BU$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
  ...
  # NEW 8/01/2025
              (1-SIM_SCENARIO_EST_CAL_FACTOR_BU$Simulated_Policy_Exempt[27])*vat_rate_on_residential_construction
            ...
            CPA_PRODUCTS_EST_TE_BU = copy(CPA_PRODUCTS_BU)
            ...
            CPA_PRODUCTS_EST_TE_BU$Est_Rev <- EST_REV_EST_TE %>% 
              dplyr::group_by(PRODUCT_INDUSTRY_CODE, PRODUCT_INDUSTRY_NAME) %>%
              dplyr::summarise(Total_Revenues_from_Intermediate_Inputs = sum(value, na.rm = T)) %>%
              dplyr::arrange(PRODUCT_INDUSTRY_CODE)
            
            CPA_PRODUCTS_EST_TE_BU$Est_Rev <- CPA_PRODUCTS_EST_TE_BU$Use_K_NetPurch %>% 
              merge.data.frame(SIM_SCENARIO_EST_TE, key = "PRODUCT_INDUSTRY_NAME") %>%
              dplyr::mutate(Final_Demand_HH = (Simulated_Policy_Reduced_Rate*PreferentialVATRate_1+Simulated_Policy_Fully_Taxable*StandardVATRate)*
                              Final_consumption_expenditure_by_households,
                            Final_Demand_NPISH = (Simulated_Policy_Reduced_Rate*PreferentialVATRate_1+Simulated_Policy_Fully_Taxable*StandardVATRate)*
                            ...
                            Final_Demand_Government = (Simulated_Policy_Reduced_Rate*PreferentialVATRate_1+Simulated_Policy_Fully_Taxable*StandardVATRate)*
                              Final_consumption_expenditure_by_government) %>%
              merge.data.frame(CPA_PRODUCTS_EST_TE_BU$Est_Rev, key = "PRODUCT_INDUSTRY_NAME") %>%
              dplyr::select(PRODUCT_INDUSTRY_CODE, PRODUCT_INDUSTRY_NAME, Total_Revenues_from_Intermediate_Inputs, Final_Demand_HH, Final_Demand_NPISH, Final_Demand_Government) %>%
              dplyr::arrange(PRODUCT_INDUSTRY_NAME)
            
            
            CPA_PRODUCTS_EST_TE_BU$Est_Rev$Final_Demand_HH[CPA_PRODUCTS_EST_TE_BU$Est_Rev$PRODUCT_INDUSTRY_NAME == "Constructions and construction works"] <- 
              CPA_PRODUCTS_EST_TE_BU$Use_K_NetPurch$Final_consumption_expenditure_by_households[CPA_PRODUCTS_EST_TE_BU$Use_K_NetPurch$PRODUCT_INDUSTRY_NAME == "Constructions and construction works"]*
              #(1-SIMULATION$Simulated_Policy_Exempt[62])*vat_rate_on_residential_construction
              # NEW 7/12/2024
          ...
          # NEW 1/8/2025
            ...
            CPA_PRODUCTS_EST_TE_BU$Est_Rev$Final_Demand_Total = psum(CPA_PRODUCTS_EST_TE_BU$Est_Rev$Total_Revenues_from_Intermediate_Inputs, 
                                                             CPA_PRODUCTS_EST_TE_BU$Est_Rev$Final_Demand_HH, 
                                                             CPA_PRODUCTS_EST_TE_BU$Est_Rev$Final_Demand_NPISH, 
                                                             CPA_PRODUCTS_EST_TE_BU$Est_Rev$Final_Demand_Government, na.rm = T) 
  ...
  # 2. SIMULATION RESULTS ----  
            ...
            'DO TUKA !!! DA SE VIDI KOJ OD OVIE Results TREBA DA OSTANAT !! TIE STO NE TREBA DA SE TRGNAT
             ISTO TAKA I KAJ OVIE MOZE DA SE NAPRAVAT IZMENI I DA NE SE KORISTAT BROEVI KAKO SUFIKSI
              ...
              CPA_PRODUCTS_1->CPA_PRODUCTS_EST_TE_BU
            ...
            # NEW
            Results_EST_TE$VAT_Gap <- as.data.frame(sum(CPA_PRODUCTS_EST_TE_BU$BM_Rev$Final_Demand_Total))
            colnames(Results_EST_TE$VAT_Gap) <- "Benchmark_VAT_LCU"
            
            Results_EST_TE$VAT_Gap$Uncalibrated_VAT_Est.LCU <- sum(CPA_PRODUCTS_EST_TE_BU$Est_Rev$Final_Demand_Total, na.rm = T)
            Results_EST_TE$VAT_Gap$Calibrated_VAT_Est.LCU <- sum(CPA_PRODUCTS_EST_TE_BU$Est_Rev$Final_Demand_Total, na.rm = T)*Locked_Calibration_Factor_BU
            Results_EST_TE$VAT_Gap$VAT_Control_Total.LCU <- sum(CPA_PRODUCTS_EST_TE_BU$Use_VAT$Total_use_at_basic_prices, na.rm = T) - sum(CPA_PRODUCTS_EST_TE_BU$Use_VAT$Exports_FOB, na.rm = T)
            ...
            # Old
            #Est_Rev1<-CPA_PRODUCTS_EST_TE_BU$Est_Rev
            # New 22/04/2024
            #Est_Rev1<-CPA_PRODUCTS_EST_TE_BU$Est_Rev
